...
- The buyer agent will use Dappier's create-account-and-login tool to create an account with Dappier using a session token generated by Ory
...
Ory is the default CIAM being used in this demo. Take a look at our [partnership with Ory](https://www.ory.sh/blog/enabling-the-agentic-economy-with-ory-and-skyfire) integrating KYAPay with CIAMs to secure the Dappier MCP server (the seller of the data).
...
4. Use the Project ID, Project Slug, API endpoints etc details from Project Settings tab in env variables in [Dappier MCP Server](https://github.com/skyfire-xyz/skyfire-solutions-demo/tree/main/mcp-servers/dappier-seller-server).

- Git clone Ory MCP repo from [here](https://github.com/ory/mcp) and use npm link to tie [mcp-access-control](https://github.com/ory/mcp/tree/main/packages/mcp-access-control) package into Dappier MCP server. This will be a npm package in a few days.
